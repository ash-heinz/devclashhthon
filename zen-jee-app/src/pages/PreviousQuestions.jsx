...
import { getQuestionsForChapter } from '../data/questionsData.js'; // <-- Import to get real question counts
...
const subjectLayout = {
  ...
  chemistry: {
    class11: { physical: ['c_u1', 'c_u2', 'c_u4', 'c_u6'], inorganic: ['c_u9', 'c_u3', 'c_u10'], organic: ['c_u13', 'c_u14', 'c_u15'] },
    class12: { physical: ['c_u5', 'c_u7', 'c_u8'], inorganic: ['c_u10_2', 'c_u11', 'c_u12'], organic: ['c_u16', 'c_u17', 'c_u18', 'c_u19', 'c_u20'] }
  },
...
export const PreviousQuestions = () => {
  ...
  const getProgress = (subjectId, chapterId, chapterName) => {
    ...
    const checkedStates = savedChecks ? JSON.parse(savedChecks) : {};
    
    const completedCount = Object.keys(checkedStates).filter(k => checkedStates[k]).length;
    const percentage = totalQuestions === 0 ? 0 : Math.round((completedCount / totalQuestions) * 100);
    
    return { percentage, totalQuestions };
  };

  const progressColors = { physics: 'bg-sky-400', chemistry: 'bg-emerald-400', mathematics: 'bg-orange-400' };
  const glowColors = { physics: 'bg-sky-500/10', chemistry: 'bg-emerald-500/10', mathematics: 'bg-orange-500/10' };
  ...
  const renderChapterList = (chapterIds, subjectId, barColor) => (
    <div className="flex flex-col gap-3 mb-6">
      {chapterIds.map((chapterId) => {
        ...
        const { percentage, totalQuestions } = getProgress(chapterId, chapter.name);

        return (
          <div 
            key={chapterId} 
            onClick={() => navigate(`/previous-questions/${subjectId}/chapter/${chapterId}`, { state: { chapterName: chapter.name } })}
            style={getGlassStyle(255, 255, 255, 0.02, 0.06)}
            className="rounded-2xl p-4 cursor-pointer hover:bg-white/5 transition-all duration-300 flex items-center justify-between group relative z-10"
          >
  ...
  const renderChemistrySections = (gradeData, subjectId, barColor) => (
    <>
      <h4 className="text-[11px] font-bold text-emerald-300/40 uppercase tracking-widest mb-3 pl-1">Physical Chemistry</h4>
      {renderChapterList(gradeData.physical, subjectId, barColor)}
      <h4 className="text-[11px] font-bold text-emerald-300/40 uppercase tracking-widest mb-3 pl-1">Inorganic Chemistry</h4>
      {renderChapterList(gradeData.inorganic, subjectId, barColor)}
      <h4 className="text-[11px] font-bold text-emerald-300/40 uppercase tracking-widest mb-3 pl-1">Organic Chemistry</h4>
      {renderChapterList(gradeData.organic, subjectId, barColor)}
    </>
  );
  ...
  const renderSubjectColumn = (subjectId, title) => {
    ...
    const glowColor = glowColors[subjectId];

    return (
      <div className="relative flex flex-col h-full rounded-3xl pt-2">
        <div className={`absolute inset-0 w-full h-[120%] ${glowColor} blur-[80px] rounded-[50px] pointer-events-none z-0`} />
        <div className="sticky top-0 z-20 pt-4 pb-4 mb-4 -mx-2 px-2 bg-[#000a24]/80 backdrop-blur-xl border-b border-white/5 rounded-b-xl shadow-sm">
          <h2 className={`text-2xl font-bold tracking-widest uppercase ${subject.colorText} drop-shadow-md`}>{title}</h2>
        </div>
        <div className="relative z-10 flex flex-col gap-2 pb-12">
          <div className="mb-4">
            ...
            {subjectId === 'chemistry' ? renderChemistrySections(layout.class11, subjectId, barColor) : renderChapterList(layout.class11, subjectId, barColor)}
          </div>
          <div className="mt-6">
            ...
            {subjectId === 'chemistry' ? renderChemistrySections(layout.class12, subjectId, barColor) : renderChapterList(layout.class12, subjectId, barColor)}
          </div>
        </div>
      </div>
    );
    ...
    <div className="h-screen w-full overflow-hidden flex flex-col bg-gradient-to-b from-[#000a24] to-black text-gray-100 font-sans antialiased relative">
      <nav style={getGlassStyle(255, 255, 255, 0.04, 0.1)} className="flex items-center justify-between px-10 py-4 shrink-0 rounded-b-3xl mx-3 shadow-lg z-40 relative">
        <div className="flex items-center gap-6">
          {/* UPDATED BACK BUTTON TO ROUTE TO DASHBOARD */}
          <button onClick={() => navigate('/')} className="p-2 rounded-full bg-white/5 hover:bg-white/10 transition-colors border border-white/10 text-white/70 hover:text-white">
            ←
          </button>
